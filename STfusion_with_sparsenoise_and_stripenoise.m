...
preShr = gpuArray(ones(size(Hr))); % Estimate of Sparse Noise on Reference HR
preSlr = gpuArray(ones(size(Lr))); % Estimate of Sparse Noise on Reference LR
preSlt = gpuArray(ones(size(Lt))); % Estimate of Sparse Noise on Target LR
preThr = gpuArray(ones(size(Hr))); % Estimate of Stripe Noise on Reference HR
preTlr = gpuArray(ones(size(Lr))); % Estimate of Stripe Noise on Reference LR
preTlt = gpuArray(ones(size(Lt))); % Estimate of Stripe Noise on Target LR
...
Z7 = gpuArray(ones(size(Dv(Hr))));
Z8 = gpuArray(ones(size(Dv(Lr))));
Z9 = gpuArray(ones(size(Dv(Lt))));
...
while is_converged == 0
    ...
    preShr_next = preShr - params.gamma1(3) * Z4;
    preShr_next = projFastL1Ball(preShr_next, params.etah); % line 10
    preSlr_next = preSlr - params.gamma1(4) * Z5;
    preSlr_next = projFastL1Ball(preSlr_next, params.etal); % line 11
    preSlt_next = preSlt - params.gamma1(5) * Z6;
    preSlt_next = projFastL1Ball(preSlt_next, params.etal); % line 12

    preThr_next = preThr - params.gamma1(6) * (Z4 + Dvt(Z7)); 
    preThr_next = projFastL1Ball(preThr_next, params.thetah); % line 13
    preTlr_next = preTlr - params.gamma1(7) * (Z5 + Dvt(Z8)); 
    preTlr_next = projFastL1Ball(preTlr_next, params.thetal); % line 14
    preTlt_next = preTlt - params.gamma1(8) * (Z6 + Dvt(Z9)); 
    preTlt_next = projFastL1Ball(preTlt_next, params.thetal); % line 15

    % Over-relaxed variables -- lines 16-19
    Hr_bar  = 2*preHr_next - preHr; 
    Ht_bar  = 2*preHt_next - preHt; 
    Shr_bar = 2*preShr_next - preShr; 
    Slr_bar = 2*preSlr_next - preSlr; 
    Slt_bar = 2*preSlt_next - preSlt; 
    Thr_bar = 2*preThr_next - preThr; 
    Tlr_bar = 2*preTlr_next - preTlr; 
    Tlt_bar = 2*preTlt_next - preTlt;
    ...
    Z4_next = Z4 + params.gamma2(4)*(Hr_bar + Shr_bar + Thr_bar); % line 24
    Z5_next = Z5 + params.gamma2(5)*(SB(Hr_bar) + Slr_bar + Tlr_bar); % line 25
    Z6_next = Z6 + params.gamma2(6)*(SB(Ht_bar) + Slt_bar + Tlt_bar); % line 26
    Z7_next = Z7 + params.gamma2(7)*(Dv(Thr_bar)); % line 27
    Z8_next = Z8 + params.gamma2(8)*(Dv(Tlr_bar)); % line 28
    Z9_next = Z9 + params.gamma2(9)*(Dv(Tlt_bar)); % line 29
